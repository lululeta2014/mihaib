...
FIREFOX_LAUNCHER="$LAUNCHERS_DIR/firefox.desktop"
if [ "$MB_LSB_ID" == Debian ]; then
	FIREFOX_LAUNCHER="$LAUNCHERS_DIR/iceweasel.desktop"
fi
xfconf-query -c xfce4-panel -p "/plugins/plugin-$i/items" -t string -s "$FIREFOX_LAUNCHER" -a -n
...
for (( ; j <= i; j++ ))
do
	TOP_LEFT_PANEL_ARGS="$TOP_LEFT_PANEL_ARGS -t int -s $j"
done
xfconf-query -c xfce4-panel -p "/panels/panel-$p/plugin-ids" -n $TOP_LEFT_PANEL_ARGS


# top-right panel plugins
...
# top-right panel (default size is 30)
p="$((p+1))"
xfconf-query -c xfce4-panel -p "/panels/panel-$p/position" -t string -s 'p=2;x=0;y=0' -n
xfconf-query -c xfce4-panel -p "/panels/panel-$p/size" -t uint -s 30 -n
xfconf-query -c xfce4-panel -p "/panels/panel-$p/position-locked" -t bool -s 'true' -n

# xfconf-query -c xfce4-panel -p "/panels/panel-$p/plugin-ids" -n -t int -s 1 -t int -s 2 ...
TOP_RIGHT_PANEL_ARGS=""
for (( ; j <= i; j++ ))
do
	TOP_RIGHT_PANEL_ARGS="$TOP_RIGHT_PANEL_ARGS -t int -s $j"
done
xfconf-query -c xfce4-panel -p "/panels/panel-$p/plugin-ids" -n $TOP_RIGHT_PANEL_ARGS
